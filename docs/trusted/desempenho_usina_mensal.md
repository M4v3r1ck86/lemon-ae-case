...
Consolida geração, clientes e resultados financeiros no nível mensal da usina e
calcula indicadores reutilizáveis. O desempenho usado no take rate considera
somente créditos associados a pagamentos elegíveis até D+60.
...
| `COUNTIF(flg_pago_ate_d60)` | `qtd_faturamentos_pagos_d60` | `—` | `INT64` | Não | Calculado/derivado | Faturamentos pagos dentro da maturação. | Contagem da flag D+60 |
| `COUNTIF(flg_pago_apos_d60)` | `qtd_faturamentos_pagos_apos_d60` | `—` | `INT64` | Não | Calculado/derivado | Faturamentos recuperados depois do fechamento. | Contagem da flag após D+60 |
...
| soma dos créditos elegíveis | `qtd_creditos_faturados_pagos_d60_kwh` | `NUMERIC` | `NUMERIC` | Não | Calculado/derivado | Numerador temporal do desempenho Lemon. | `SUM(qtd_creditos_faturados_pagos_d60_kwh)` |
| maior limite D+60 | `dt_fechamento_competencia` | `DATE` | `DATE` | Não | Calculado/derivado | Data de fechamento da usina e competência. | `MAX(dt_limite_pagamento_d60)` |
| fechamento e data atual | `flg_competencia_fechada` | `—` | `BOOL` | Não | Calculado/derivado | Indica maturidade para publicação. | `CURRENT_DATE() >= dt_fechamento_competencia` |
...
| soma do valor elegível | `vlr_liquidado_gerador_d60_brl` | `—` | `NUMERIC` | Não | Calculado/derivado | Parcela liquidada reconhecida no fechamento. | `SUM(vlr_liquidado_gerador_d60_brl)` |
| soma da recuperação posterior | `vlr_liquidado_gerador_apos_d60_brl` | `—` | `NUMERIC` | Não | Calculado/derivado | Parcela recebida após a maturação. | `SUM(vlr_liquidado_gerador_apos_d60_brl)` |
| juros elegíveis | `vlr_juros_pago_d60_brl` | `—` | `NUMERIC` | Não | Calculado/derivado | Juros recebidos até D+60. | soma condicionada pela flag D+60 |
| multas elegíveis | `vlr_multa_paga_d60_brl` | `—` | `NUMERIC` | Não | Calculado/derivado | Multas recebidas até D+60. | soma condicionada pela flag D+60 |
...
| `SAFE_DIVIDE(qtd_creditos_faturados_pagos_d60_kwh, qtd_minima_injecao_kwh)` | `perc_desempenho_lemon` | `—` | `NUMERIC` | Não | Calculado/derivado | Razão entre créditos faturados pagos até D+60 e a quantidade mínima de injeção. | Divisão decimal segura no fechamento |
